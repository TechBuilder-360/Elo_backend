...
func (vq *VaultQuery) QueryOwner() *LedgerOwnerQuery {
	query := (&LedgerOwnerClient{config: vq.config}).Query()
...
func (vq *VaultQuery) QueryWallets() *WalletQuery {
	query := (&WalletClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VaultQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (vq *VaultQuery) FirstIDX(ctx context.Context) string {
...
func (vq *VaultQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (vq *VaultQuery) OnlyIDX(ctx context.Context) string {
...
func (vq *VaultQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (vq *VaultQuery) IDsX(ctx context.Context) []string {
	ids, err := vq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (vq *VaultQuery) Clone() *VaultQuery {
	...
	return &VaultQuery{
		...
		withWallets: vq.withWallets.Clone(),
...
func (vq *VaultQuery) WithOwner(opts ...func(*LedgerOwnerQuery)) *VaultQuery {
	query := (&LedgerOwnerClient{config: vq.config}).Query()
...
func (vq *VaultQuery) WithWallets(opts ...func(*WalletQuery)) *VaultQuery {
	query := (&WalletClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withWallets = query
	return vq
}
...
func (vq *VaultQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vault, error) {
	...
	if query := vq.withOwner; query != nil {
		if err := vq.loadOwner(ctx, query, nodes, nil,
			func(n *Vault, e *LedgerOwner) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := vq.withWallets; query != nil {
		if err := vq.loadWallets(ctx, query, nodes,
			func(n *Vault) { n.Edges.Wallets = []*Wallet{} },
			func(n *Vault, e *Wallet) { n.Edges.Wallets = append(n.Edges.Wallets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VaultQuery) loadOwner(ctx context.Context, query *LedgerOwnerQuery, nodes []*Vault, init func(*Vault), assign func(*Vault, *LedgerOwner)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Vault)
	for i := range nodes {
		if nodes[i].ledger_owner_vaults == nil {
			continue
		}
		fk := *nodes[i].ledger_owner_vaults
	...
	query.Where(ledgerowner.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ledger_owner_vaults" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (vq *VaultQuery) loadWallets(ctx context.Context, query *WalletQuery, nodes []*Vault, init func(*Vault), assign func(*Vault, *Wallet)) error {
	...
	query.Where(predicate.Wallet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vault.WalletsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.vault_wallets
		if fk == nil {
			return fmt.Errorf(`foreign-key "vault_wallets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "vault_wallets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
