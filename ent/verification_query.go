...
func (vq *VerificationQuery) QueryBusiness() *BusinessQuery {
	query := (&BusinessClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VerificationQuery) Clone() *VerificationQuery {
	...
	return &VerificationQuery{
		...
		withBusiness: vq.withBusiness.Clone(),
...
func (vq *VerificationQuery) WithBusiness(opts ...func(*BusinessQuery)) *VerificationQuery {
	query := (&BusinessClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withBusiness = query
	return vq
}
...
func (vq *VerificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Verification, error) {
	...
	if query := vq.withUser; query != nil {
		if err := vq.loadUser(ctx, query, nodes,
			func(n *Verification) { n.Edges.User = []*User{} },
			func(n *Verification, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	if query := vq.withBusiness; query != nil {
		if err := vq.loadBusiness(ctx, query, nodes,
			func(n *Verification) { n.Edges.Business = []*Business{} },
			func(n *Verification, e *Business) { n.Edges.Business = append(n.Edges.Business, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VerificationQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Verification, init func(*Verification), assign func(*Verification, *User)) error {
	...
	byID := make(map[string]*Verification)
	nids := make(map[string]map[*Verification]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(verification.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(verification.UserPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(verification.UserPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(verification.UserPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Verification]struct{}{byID[outValue]: {}}
...
func (vq *VerificationQuery) loadBusiness(ctx context.Context, query *BusinessQuery, nodes []*Verification, init func(*Verification), assign func(*Verification, *Business)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Verification)
	nids := make(map[string]map[*Verification]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(verification.BusinessTable)
		s.Join(joinT).On(s.C(business.FieldID), joinT.C(verification.BusinessPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(verification.BusinessPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(verification.BusinessPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
